...
def test_Phi_start():
  ...
  FuInType = mk_bits(clog2(num_inports + 1))
  pickRegister = [FuInType(x + 1) for x in range(num_inports)]
               # Two PHI_START ops can share a tile but live at different ctrl addrs.
               # Each ctrl slot should take src0 on its first execution;
               # later executions follow the original predicate selection rule.
  src_in0 =   [DataType(2, 1), DataType(3, 0), DataType(6, 0), DataType(3, 1)]
  src_in1 =   [                                DataType(8, 1), DataType(5, 0)]
  src_const = []
               # Execute ctrl addrs 0 and 1 twice: 0, 1, 0, 1.
...
def test_Phi_const():
  ...
  FuInType = mk_bits(clog2(num_inports + 1))
  pickRegister = [FuInType(x + 1) for x in range(num_inports)]
               # Two PHI_CONST ops can share a tile but live at different ctrl addrs.
               # Clearing first[0] must not clear first[1]; otherwise the
               # second PHI_CONST would take src_in0=2 instead of const=6.
