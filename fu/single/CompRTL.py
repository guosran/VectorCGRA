...
class CompRTL(Fu):
  ...
  def construct(s, CtrlPktType, num_inports, num_outports, vector_factor_power = 0):
    ...
    super(CompRTL, s).construct(CtrlPktType, num_inports, num_outports, 1, vector_factor_power)
    
    num_entries = 2
    s.const_one = s.DataType(1, 0)
    FuInType = mk_bits(clog2(num_inports + 1))
    CountType = mk_bits(clog2(num_entries + 1))

    # For signed comparison: XOR with MSB flips signed to unsigned order
    PayloadBitsType = s.DataType.get_field_type('payload')
    payload_nbits = PayloadBitsType.nbits
    sign_flip = PayloadBitsType(1 << (payload_nbits - 1))
      ...
      if s.recv_opt.val:
        if (s.recv_opt.msg.operation == OPT_EQ) | (s.recv_opt.msg.operation == OPT_NE):
          ...
          if (s.recv_opt.msg.operation == OPT_EQ_CONST) & \
            ...
            s.send_out[0].msg @= s.const_zero
          # EQ/NE_CONST is used to generate loop-control predicates. The
          # comparison result remains valid even when the incoming data token
          # carries a false predicate at the terminal iteration.
        ...
        elif s.recv_opt.msg.operation == OPT_LT_CONST:
          # Signed less-than: XOR MSB converts signed order to unsigned order
          if (s.recv_in[s.in0_idx].msg.payload ^ sign_flip) < (s.recv_const.msg.payload ^ sign_flip):
            s.send_out[0].msg @= s.const_one
          else:
            s.send_out[0].msg @= s.const_zero
