...
class CtrlMemDynamicRTL(Component):
  ...
  def construct(s, IntraCgraPktType,
    ...
    def update_send_ctrl():
      ...
      for i in range(num_routing_outports):
        s.send_ctrl.msg.routing_xbar_outport[i] @= \
            s.reg_file.rdata[0].routing_xbar_outport[i]
        s.send_ctrl.msg.fu_xbar_outport[i] @= \
            s.reg_file.rdata[0].fu_xbar_outport[i]
      s.send_ctrl.msg.vector_factor_power @= \
          s.reg_file.rdata[0].vector_factor_power
      # Concrete example: when total_ctrl_steps is 4, the dynamic
      # control issued at times==3 is the final real control word. Mark
      # that word as last so RET observes the normal return path instead
      # of relying on a synthetic timeout COMPLETE packet at times==4.
